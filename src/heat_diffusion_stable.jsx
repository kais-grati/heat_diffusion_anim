...
const HeatDiffusionVisualization = () => {
  ...
  const getInitialCondition = (x) => {
    ...
    switch (initialCondition) {
      ...
      case 'step-discontinuous':
        ...
        break;
      case 'triangle':
        if (x >= 4 && x <= 5) value = (x - 4);
        else if (x > 5 && x <= 6) value = (6 - x);
        else value = 0;
        break;
      case 'two-peaks':
        value = Math.exp(-((x - 3) ** 2) / 0.3) + Math.exp(-((x - 7) ** 2) / 0.3);
        break;
      case 'sigmoid':
        value = 1 / (1 + Math.exp(-2 * (x - 5)));
        break;
      default:
        value = 0;
    }
  ...
  useEffect(() => {
    ...
    if (barType === 'infinite' && time > 0) {
      ...
      const kernelCenter = 5; // Center of domain
      for (let i = 0; i < numPoints; i++) {
        const x = xMin + (i / (numPoints - 1)) * (xMax - xMin);
  ...
  }, [time, alpha, barType, initialCondition, boundaryCondition]);
  ...
  return (
    <div className="w-full h-screen bg-gray-900 p-6 flex flex-col">
      <div className="text-white mb-4">
        ...
        <p className="text-gray-400">Observe how heat spreads via Gaussian convolution (infinite) or Fourier series (finite)</p>
      </div>
